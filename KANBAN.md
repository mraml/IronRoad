...
- [x] **~20 event templates** — all major event kinds represented §6.1 _(now 60+ events including npc_conversation kind)_
- [x] **7 historical anchors + 1 elite stub + 3 foot beats** §2.6, §8.2 _(now 10 anchors, 5 elites, 10 foot beats — see Wave 7-B + Phase 1)_
...
- [x] **All 7 original anchor events** expanded to 2–3 choices (commander/driver/loader angles) _(3 more anchors added in Wave 7-B, each authored with 3 choices)_
...
### Phase 3 — KANBAN refresh _(prior pass)_
...
## Done — Operation Health (maintenance)

- [x] **ESLint + Prettier + CI** — `eslint.config.js`, format/lint/typecheck scripts, CI steps
- [x] **Bookend prose guards** — duplicate sentence lint, milestone NPC lint, briefing orders lint, mission brief token resolution
- [x] **Campaign smoke tests** — seeded bookend + auto-play harness in `campaignSmoke.test.ts`
- [x] **Prose dedupe** — final_push stack, opener/framing split, briefing `preChoiceNpc` trims, milestone `{briefer}` continuity
- [x] **Engine cleanup** — `encounterDiceMods.ts`, shared combat kind list, remove dead `accumulatedEffects`, seeded `PICK_TANK`, `footMode` in tactical outcome text
- [x] **Save validation** — `saveValidation.ts` on rehydrate + `LOAD_STATE`; template safety tests
