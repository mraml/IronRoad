...
7. **Beat separation (mission start)** — `{briefer}` + `{briefingPlace}` introduced **once** on the framing slide only; archetype slides continue with *he* / *the officer* (no second intro, no `{objective}` on framing); interactive `briefing_*` narrative is **orders-only** (timing, constraints, `{objective}`) — no map/grease re-description before choices.
