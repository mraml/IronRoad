...
**Diversity / coverage check:** `npm test -- src/engine/generator.test.ts src/content/poolKinds.test.ts src/content/eventsCatalog.test.ts src/content/catalogProseLint.test.ts src/content/starProseLint.test.ts src/content/starTravelSupply.test.ts src/content/starCombatAnchors.test.ts src/content/missionBrief.test.ts src/engine/reducer.narrativeFlow.test.ts src/engine/campaignCalendar.test.ts src/engine/reducer.encounterDepth.test.ts src/engine/reducer.trauma.test.ts` — Tier-1 ≥125, Tier-2 ≥55, combined ≥180, anchors ≥20, social ≥20, STAR prose patch compliance, mission brief + area entry flow, disjoint pools, encounter follow-up depth, Fury tier-2 on second pass, season-env matrix on all mission days, `measureFillerCoverage`, kind-mix per mission, seeded foot beat order.
